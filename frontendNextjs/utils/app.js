...
void main(){
    ...
    float PI = 3.1415925;
    // float noise = cnoise(vec3(position.x*4., position.y*4. + time/50.,0.));
    // float noise = cnoise(3.*vec3(position.x,position.y,position.z +time/3 0.));
...
export default class Sketch {
  constructor(options) {
    ...
    this.height = this.container && this.container.offsetHeight;
    ...
    this.iamges = [...document.querySelectorAll('img')];
    ...
    Promise.all(allDone).then(() => {
      ...
      // //  this.currentSrcoll = window.scrollY;
      //    this.setPosition();
      //  })
    })
  ...
  addImages(){
    ...
    this.materials = [];
    this.imageStore = this.iamges.map(img => {
  ...
  setPosition(){
    this.imageStore.forEach(o=> {
      o.mesh.position.y= this.currentSrcoll -o.top + this.height/2 - o.height/2;
      o.mesh.position.x= o.left - this.width/2 + o.width/2;
      // console.log(o.mesh.position.x, o.mesh.position.y, 'element');
    })
  }

  addObjects() {
    this.geometry = new THREE.PlaneGeometry(200, 400, 10, 10);
    // this.geometry = new THREE.SphereBufferGeometry(0.4 , 30, 30);
  ...
  render() {
    this.time += 0.05;
    this.scroll.render();
    this.currentSrcoll = this.scroll.scrollToRender;
    this.setPosition();
